...
export class EntityRepository implements IEntityRepository {
  ...
  async findAll(page = 1, limit = 20): Promise<EntityDto[]> {
    ...
    try {
      const [rows] = await res.conn.query<RowDataPacket[]>(
        `SELECT * FROM tournaments ORDER BY created_at DESC LIMIT ${safeLimit} OFFSET ${safeOffset}`
      );
  ...
  async findByGameId(gameId: number): Promise<EntityDto[]> {
    ...
    try {
      const [rows] = await res.conn.execute<RowDataPacket[]>(
        "SELECT * FROM tournaments WHERE game_id = ? ORDER BY created_at DESC",
        [gameId]
      );
  ...
  async create(dto: CreateEntityDto): Promise<Entity> {
    ...
    try {
      ...
      return new Entity(result.insertId, dto.game_id, EntityStatus.UPCOMING);
    } catch (err) {
